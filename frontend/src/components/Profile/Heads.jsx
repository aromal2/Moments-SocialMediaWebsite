...
function Heads() {
  ...
  console.log(userId.Id,"7777777771111111");
  ...
  let { userName, profilePhoto, email} = useSelector(
    (state) => state.user
  );
  ...
  const postResponse = async () => {
    const postResponse = await singleuserPost(userId.Id);
    setPostresponse(postResponse);
  };

  const userResponse = async () => {
    const userResponse = await singleUserdetails(userId.Id);
    console.log(userResponse, "66useresponseee");
          ...
          <div className="flex ms-32">
            <div className="">
              <p className="mb-8 text-blue-gray-700">
                {postResponses.length} posts
              </p>
              {verifybutton ? (
                <div className="flex items-center">
                  <p className="mb-2 text-black ms-2">{userName}</p>
            ...
            <div className="ms-14">
              <div
                className="mb-8 text-blue-gray-700 ms-4"
            ...
            <div className="ms-14 ">
              <p
                className="mb-8 text-blue-gray-700"
              ...
              <Button
                variant="gradient"
                className="rounded-full -mt-2 me-1"
                size="sm"
              >
                archive
              </Button>
